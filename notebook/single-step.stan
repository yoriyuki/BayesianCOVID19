functions{
  ...
  real poisson_approx_cum(real k, real lambda){
    return normal_lcdf(k+0.5 | lambda, sqrt(fmax(lambda,0.1)));
  }

  real poisson_approx_pmf(real k, real lambda){
    return logdiff(poisson_approx_cum(k+1, lambda), poisson_approx_cum(k, lambda));
  }
